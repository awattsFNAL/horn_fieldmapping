...
import time
import numpy as np
import scope_communication as scopecom

# Move parameters to a config file in the future
...
def setup():
    ...
    stripline_scope = scopecom.initialize_scope(stripline_scope_IP)

    # Setup field scope
    print 'Initializing magnetic field scope.'
    field_scope = scopecom.initialize_scope(field_scope_IP)
...
def get_data(stripline_scope, field_scope,X_pos, Y_pos, Z_pos):
    ...
    while point != (npoints+1):
        ...
        stripline_time2, stripline_voltage1 = get_waveform(stripline_scope_type, stripline_scope, 2)
        stripline_time3, stripline_voltage1 = get_waveform(stripline_scope_type, stripline_scope, 3)
        stripline_time4, stripline_voltage1 = get_waveform(stripline_scope_type, stripline_scope, 4)
